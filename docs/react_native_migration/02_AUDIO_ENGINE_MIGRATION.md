# Phase 2: Audio Engine Manager Migration

## Overview

The `AudioEngineManager` is largely framework-agnostic and can be migrated with minimal changes. It manages `AVAudioEngine` for low-latency audio recording, waveform level calculation, and audio preservation.

## Migration Strategy

**Key Insight:** The existing Swift code has no Flutter dependencies in the core audio logic. Only the delegate pattern for event notification needs slight adaptation.

### Changes Required

| Aspect | Change Type | Description |
|--------|-------------|-------------|
| Delegate callbacks | Minor | Keep protocol, coordinator will adopt |
| Logging | Optional | Replace `print()` with RN-compatible logging |
| File imports | None | All iOS frameworks remain the same |
| Core logic | None | `AVAudioEngine`, `AVAudioSession` unchanged |

## Implementation

### 1. Copy AudioEngineManager.swift

Copy the existing file from the Flutter project with these modifications:

**AudioEngineManager.swift** (React Native version)
```swift
import AVFoundation
import AudioToolbox
import Foundation
...
- All `AVAudioEngine` and `AVAudioSession` logic
- Buffer processing and level calculation
- Audio preservation pipeline
- Interruption handling
- State machine

### What Changed
- Logging: Removed verbose Flutter-specific logs, simplified for React Native
- Dispatch queue labels: Changed from `com.flutterdictation` to `com.reactnativedictation`
- Removed excessive debug prints (keep them for development, remove for production)

## Testing

1. Create a simple test in the coordinator:
```swift
// In DictationCoordinator
func testAudioEngine() async throws {
...
- [ ] `AudioEngineManager.swift` compiles without Flutter imports
- [ ] Microphone permission request works on main thread
- [ ] Audio level calculation returns values 0.0-1.0
- [ ] Buffer callback is invoked when recording
- [ ] Audio preservation creates valid .m4a files
- [ ] Interruption handling pauses/resumes correctly

## Next Steps

Proceed to [03_SPEECH_RECOGNIZER_MIGRATION.md](./03_SPEECH_RECOGNIZER_MIGRATION.md) to migrate the SpeechRecognizerManager.
